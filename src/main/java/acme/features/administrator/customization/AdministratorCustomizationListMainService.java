...
@Service
public class AdministratorCustomizationListMainService implements AbstractListService<Administrator, Customization> {
	...
	public Collection<Customization> findMany(final Request<Customization> request) {
		assert request != null;
		Collection<Customization> custs;
		custs = this.repository.findMany();
		Collection<Customization> result = new HashSet<Customization>();
		result.add(custs.iterator().next());
		return result;
	}
